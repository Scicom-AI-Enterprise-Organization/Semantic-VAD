...
a **true end of turn**. This package turns forced alignments (words with start/end times)
into a labeled benchmark for exactly that decision.

> This is the dataset-builder half of the Semantic-VAD project — it produces
> [`Scicom-intl/semantic-vad-eot`](https://huggingface.co/datasets/Scicom-intl/semantic-vad-eot),
> the training data the model in [`semvad/`](../semvad) (repo root) is fine-tuned on and
> evaluated against. See the [top-level README](../README.md) for the trained model, its
> benchmark results, and how to run/evaluate it.
...
> **Note (repo root):** the model actually trained and benchmarked in this repo
> (`semvad/`) took a different path than the one described above — it fine-tunes
> `Qwen2-Audio-7B-Instruct` directly (its own Whisper-style audio tower + adapter, no
> separate Qwen3 swap) with a classification head instead of a marker-token read. See the
> [top-level README](../README.md) for what was actually built and its results; the
> Whisper→adapter→Qwen3 design below is this sub-project's original architecture plan.
